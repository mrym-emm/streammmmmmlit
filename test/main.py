...
# Title
st.markdown(
    '<div class="title">Port Dickson AQI Prediction</div>', unsafe_allow_html=True
)
st.markdown(
    '<div class="subtitle">Forecast air quality using Prophet model</div>',
    unsafe_allow_html=True,
)
...
def load_model():
    try:
        ...
        possible_paths = [
            "prophet_aqi_model.pkl",
            "./prophet_aqi_model.pkl",
            "../prophet_aqi_model.pkl",
            "test/prophet_aqi_model.pkl",
        ]
        ...
        raise FileNotFoundError(
            f"Couldn't find model file in any of the tried paths: {possible_paths}"
        )
...
if model_data:
    ...
    max_date = last_date + timedelta(
        days=365
    )  # Allow predictions up to 1 year in the future
    ...
    if len(historical_temps) > 0:
        ...
        monthly_temps = historical_data[historical_data["ds"].dt.month == target_month][
            "temperature"
        ]
        avg_temp = (
            monthly_temps.mean() if len(monthly_temps) > 0 else 28.0
        )  # Default if no data

    if use_custom_temp:
        temperature = st.number_input(
            "Enter expected temperature (°C):",
            min_value=15.0,
            max_value=40.0,
        ...
        st.write(
            f"Using historical average temperature for {target_date.strftime('%B %d')}: {avg_temp:.2f}°C"
        )
    ...
    if st.button("Generate Prediction"):
        with st.spinner("Generating forecast..."):
            ...
            future = future.merge(
                historical_data[["ds", "temperature"]], on="ds", how="left"
            )
            ...
            for idx, row in future[future["temperature"].isna()].iterrows():
                ...
                if (month, day) in target_month_avg_temps.index:
                    future.loc[idx, "temperature"] = target_month_avg_temps.loc[
                        (month, day)
                    ]
                else:
                    # Fallback to month average
                    month_avg = historical_data[
                        historical_data["ds"].dt.month == month
                    ]["temperature"].mean()
                    future.loc[idx, "temperature"] = (
                        month_avg if not np.isnan(month_avg) else avg_temp
                    )
            ...
            if not target_forecast.empty:
                ...
                st.markdown(
                    f"<h3 style='color:{color}'>Air Quality: {category}</h3>",
                    unsafe_allow_html=True,
                )
    ...
    st.error(
        "Failed to load the model. Please ensure the model file exists and is correctly formatted."
    )
